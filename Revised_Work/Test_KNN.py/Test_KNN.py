...
xte=tf.placeholder(tf.float32,[None, 28*28]) #testing input
...
with tf.Session() as sess:
    ...
    for i in range(X_test.shape[0]):
        ...
        predicted_value=sess.run(pred,feed_dict={xtr:X_traning,ytr:Y_traning,xte:X_test})

        print("Test",i,"Prediction",predicted_value,"True Class:",np.argmax(Y_test[i]))

        if predicted_value == np.argmax(Y_test):
