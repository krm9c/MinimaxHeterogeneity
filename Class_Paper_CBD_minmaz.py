...
def init_ftn(name, num_input, num_output, runiform_range):
    if(name == "normal"):
        ...
        return(tf.random_uniform([num_input, num_output], \
        minval=-runiform_range, maxval=runiform_range))
    else:
...
class learners():
    def __init__(self):
        ...
        self.Evaluation = {}
        self.keys = []
        self.sess = tf.Session(config=get_session_config(10))
        ...
        with tf.name_scope(key):
            ...
            with tf.name_scope('Wx_plus_b' + key):
                preactivate = tf.matmul(
                    input_tensor, self.classifier['Weight' + key])\
                    + self.classifier['Bias' + key]
...
################################################################################################################           
    def New_Formulation(self, classes, Layers, act_function, batch_size, rho = 0.8):
 ...
 ############################################################################################################################
    def init_NN_custom(self, classes, lr, Layers, act_function, batch_size, gamma,
                par='GDR', act_par="tf.nn.tanh", rho = 0.1):
################################################################## Initial Definitions
...
##################################################################################### The network
                ...
                self.New_Formulation(classes, Layers, act_function, batch_size, rho)
...
############################################################## Design the trainer for the  network
            ...
            #The overall cost function
                ...
                self.classifier["KL"] = (np.square(gamma))*(self.classifier["Cross_Entropy"])
